...
namespace ForQab.Service
{
    public class MonitorService : IMonitorService
    {
        ...
        "MonitorsProfessions.SubProfession", "MonitorLogs", "Contracts", "ExamMonitors"
            };
     ...
     List<int> selectedIds, string searchName, int? districtId)
        ...
        public async Task<(List<Monitor> Items, int TotalCount)> GetPagedAsync(
    ...
    DateTime? createdStartDate, DateTime? createdEndDate, int page, int pageSize)
        {
            ...
            if (genderId.HasValue && genderId > 0)
                query = query.Where(m => m.Gender == genderId.Value);
            ...
            if (!string.IsNullOrEmpty(finCode))
                query = query.Where(m => EF.Functions.Like(m.FinCode, $"%{finCode}%"));

            if (!string.IsNullOrEmpty(serial))
                query = query.Where(m => EF.Functions.Like(m.Serial, $"%{serial}%"));

            if (district.HasValue && district > 0)
                query = query.Where(m => m.District == district.Value);

            if (startYear.HasValue)
                query = query.Where(m => m.BirthDate.HasValue && m.BirthDate.Value.Year >= startYear.Value);

            if (endYear.HasValue)
                query = query.Where(m => m.BirthDate.HasValue && m.BirthDate.Value.Year <= endYear.Value);

            if (createdStartDate.HasValue)
                query = query.Where(m => m.CreatedAt.HasValue && m.CreatedAt.Value.Date >= createdStartDate.Value.Date);

            if (createdEndDate.HasValue)
                query = query.Where(m => m.CreatedAt.HasValue && m.CreatedAt.Value.Date <= createdEndDate.Value.Date);
            ...
            var items = await query
                ...
                .Include(m => m.ExamMonitors)
                    .ThenInclude(em => em.Exams)   // ThisYearAssignmentCount üçün
                .AsSplitQuery()
                .ToListAsync();
